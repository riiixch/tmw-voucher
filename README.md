...
- **ปัญหา**: เบอร์โทรศัพท์ของผู้โอนเงินมักติดขีดแดช (`-`), ช่องว่าง, หรือส่งมาในรูปแบบรหัสสากล (เช่น `+66812345678` หรือ `66812345678`) ซึ่งจะทำให้ระบบทรูมันนี่ปฏิเสธคำขอการรับเงิน
- **วิธีแก้**: ระบบติดตั้งฟีเจอร์ **International Phone Normalization** ช่วยล้างอักขระพิเศษ และแปลงรหัส `+66` / `66` นำหน้าให้เป็น `0` นำหน้าโดยอัตโนมัติ (เช่น `+66 81-234-5678` -> `0812345678`) พร้อมบังคับการตรวจสอบความยาว 10 หลักอย่างรัดกุมก่อนส่งออก
...
// 1. กำหนดอินสแตนซ์พร้อมเบอร์โทรและเวลาตัดขาดสัญญาณ (Timeout)
const redeemer = new TmwVoucher({
  mobile: '081-234-5678', // ระบบจะทำการ Normalize เป็น "0812345678" ให้อัตโนมัติ
...
| :--- | :--- | :---: | :---: | :--- |
| `mobile` | `string` | No | - | เบอร์โทรศัพท์ 10 หลักสำหรับรับเงิน (รองรับการ Normalize เคลียร์ขีดช่องว่างและรหัสประเทศให้เสร็จสรรพ) |
...
| :--- | :--- |
| `TmwValidationError` | ข้อมูลนำเข้าไม่ถูกต้องตามเงื่อนไข (เช่น ไม่ได้ระบุเบอร์ หรือเบอร์ไม่ครบ 10 หลักถ้วนหลังการ Normalize) |
...
| `TmwNetworkError` | ปัญหาการรับส่งแพ็กเก็ตข้อมูล, ระบบ curl ภายในพัง หรือไม่พบไฟล์ปฏิบัติการใน PATH |
